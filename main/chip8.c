...
{
    if (!rom || size == 0 || size > CHIP8_MAX_ROM) return false;
...
{
    if (seed) C.rng = seed;            // a xorshift 0-ból nem mozdul ki
}
...
{
    if (key >= 0 && key < CHIP8_KEYS) C.keys[key] = down;
}
...
bool chip8_beeping(void)       { return C.st > 0; }
bool chip8_halted(void)        { return C.halted; }
const uint8_t *chip8_fb(void)  { return C.fb; }
